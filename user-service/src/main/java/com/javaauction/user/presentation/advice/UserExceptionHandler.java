...
@Slf4j
@RestControllerAdvice(basePackages = {"com.javaauction.user"})
...
//    DB 중복 Key 에러 처리
    ...
    public ResponseEntity<ErrorResponse> handleDuplicate(DataIntegrityViolationException e) {
        ...
        if (msg.contains("username")) {
            return build(UserErrorCode.USER_ALREADY_EXISTS);
        }
        // email
        if (msg.contains("email")) {
            return build(UserErrorCode.EMAIL_ALREADY_EXISTS);
        }
        // slack
        if (msg.contains("slack_id") || msg.contains("slack")) {
            return build(UserErrorCode.SLACK_ID_ALREADY_EXISTS);
        }
